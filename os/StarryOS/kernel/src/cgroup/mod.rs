...
use alloc::{collections::{BTreeMap, BTreeSet}, format, string::{String, ToString}, sync::Arc, vec::Vec};
use ::core::fmt::Write;
use ::core::str;
...
const CONTROLLER_ATTRS: &[AttrInfo] = &[
    AttrInfo { name: "pids.max", read_only: false },
    AttrInfo { name: "pids.current", read_only: true },
    AttrInfo { name: "cpu.weight", read_only: false },
    AttrInfo { name: "cpu.max", read_only: false },
    AttrInfo { name: "cpu.stat", read_only: true },
];
...
pub fn write_subtree_control(id: CgroupId, data: &[u8]) -> VfsResult<()> {
    let membership = MEMBERSHIP.get().ok_or(VfsError::BadState)?.lock();
    let node = core::get_node(id)?;
    let text = str::from_utf8(data).map_err(|_| VfsError::InvalidInput)?.trim();
...
fn has_domain_subtree_control(node: &CgroupNode) -> bool {
    node.subtree_control.lock().iter().any(|controller| is_domain_controller(controller))
}
...
pub fn migrate_process(pid: u32, target_id: CgroupId) -> VfsResult<()> {
    ...
    let pd = crate::task::get_process_data(pid as _).map_err(|_| VfsError::NotFound)?;
    if membership.detached_pids.contains(&pid) || pd.proc.is_zombie() {
        return Err(VfsError::NoSuchProcess);
    }

    let old = pd.cgroup.write().clone();
    if old.id == target.id {
        return Ok(());
    }
    if !old.procs.lock().iter().any(|&member| member == pid) {
        return Err(VfsError::NoSuchProcess);
    }
    ...
    *pd.cgroup.write() = target;
    membership.detached_pids.remove(&pid);
    uncharge_path(&old_path[..old_unique_len]);
    Ok(())
}

pub fn exit_process(process: &crate::task::ProcessData, pid: u32) -> VfsResult<()> {
    let mut membership = MEMBERSHIP.get().ok_or(VfsError::BadState)?.lock();
    let cgroup = process.cgroup.read().clone();
...
fn controller_available(node: &CgroupNode, name: &str) -> bool {
    ...
    node.parent
        ...
        .is_some_and(|parent| parent.subtree_control.lock().iter().any(|controller| controller == name))
}
...
fn write_cpu_max(node: &CgroupNode, text: &str) -> VfsResult<()> {
    ...
    let quota = if parts[0] == "max" {
        ...
        let quota = parts[0].parse::<i64>().map_err(|_| VfsError::InvalidInput)?;
        if quota <= 0 {
            return Err(VfsError::InvalidInput);
        }
        quota
