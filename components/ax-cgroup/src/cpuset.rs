...
impl CpusetState {
    pub fn new() -> Self {
        // Default: all CPUs and memory nodes allowed
        Self {
...
impl CgroupController for CpusetController {
    ...
    fn write_attr(&self, name: &str, data: &[u8]) -> VfsResult<usize> {
        ...
        match name {
            "cpus" => {
                let mask = CpusetState::parse_cpu_list(text)
                    .map_err(|_| VfsError::InvalidInput)?;
                self.state.cpus.store(mask, Ordering::Release);
                // Update effective mask (simplified: no parent intersection here)
